...
# ====== CONFIG ======
BOT_TOKEN = "8788508823:AAELUW3TFS3_iyH5yhfJhLRrtHH5iSIISV4"
...
if __name__ == "__main__":
    main()            "https://api.internal.temp-mail.io/api/v3/email/new",
            data=json.dumps({"min_name_length":10,"max_name_length":10}).encode(),
            headers={"Content-Type":"application/json"}
        )
        res = json.loads(urllib.request.urlopen(req).read())
        return res.get("email"), res.get("token")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    run_web()            data=json.dumps({"min_name_length":10,"max_name_length":10}).encode(),
            headers={"Content-Type":"application/json"}
        )
        res = json.loads(urllib.request.urlopen(req).read())
        return res["email"], res["token"]
